...
def process_file(file_path):
    try:
        ...
        # 旧形式 (div with hr, or simple paragraphs)
        ...
        # 3. 再挿入
        # 上部：<h1> の直後、または <div class="container"> の直後
        if '</h1>' in content:
        ...
        if '</style>' in content:
            if '.page-nav' not in content:
                ...
                # 既にスタイルがある場合は、最新のものに置き換え（正規表現の微調整）
                style_pattern = re.compile(r'/\* モダンなナビゲーションボタン用スタイル \*/.*?@media.*?\.nav-btn\s*\{.*?\}', re.DOTALL)
                if style_pattern.search(content):
                    content = style_pattern.sub(NAV_STYLE, content)
                else:
                    # パターンに合致しないがスタイル名はある場合、末尾に追加して重複を避ける（本来はもっと厳密にするべきだが）
                    pass
...
def main():
    ...
    print(f"Found {len(files_to_process)} content files to apply global navigation update...")
